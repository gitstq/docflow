...
def convert(
    ...
    with console.status("[bold green]Converting...") as status:
        if source_path.is_file():
            ...
            if result.success:
                console.print(f"[green]✓[/green] Converted: {result.source_path}")
                ...
                console.print(f"[red]✗[/red] Failed: {result.source_path}")
...
def batch(
    ...
    if result.successful + result.partial > 0:
        ...
        for r in result.results:
            if r.success:
                status = "[green]✓[/green]" if r.status.value == "success" else "[yellow]◐[/yellow]"
